    ...
    const handleSelect = async (id) => {
        try {
            const updated = watchlistIds.includes(id)
                ? watchlistIds.filter(i => i !== id)
                : [...watchlistIds, id];

            const resp = await axios.put(
                `${API_BASE}/api/users/${encodeURIComponent(user)}/watchlist`,
                { watchlist: updated }
            );
            ...
            // Remove from wishlist if added to watchlist
            if (updated.includes(id) && wishlistIds.includes(id)) {
                const newWishlist = wishlistIds.filter(i => i !== id);

                try {
                    const r2 = await axios.put(
                        `${API_BASE}/api/users/${encodeURIComponent(user)}/wishlist`,
                        { wishlist: newWishlist }
                    );

                    if (r2.status !== 200) {
                        console.error('Failed to persist wishlist to server', r2.status);
                    }
                } catch (e) {
    ...
    useEffect(() => {
        ...
        const fetchSavedAnime = async () => {
            ...
            try {
                ...
                // Fetch watchlist IDs
                const wlResp = await axios.get(
                    `${API_BASE}/api/users/${encodeURIComponent(user)}/watchlist`
                );

                const savedIds =
                    wlResp.status === 200
                        ? (wlResp.data.watchlist || [])
                        : [];
                        ...
                        filteredAnime.map((item) => (
                            ...
                            <AnimeCard
                                ...
                                onToggleWish={async (id) => {
                                    ...
                                    const updated = wishlistIds.includes(id)
                                    ...
                                    try {
                                        ...
                                        if (resp.status !== 200) {
                                            console.error(
                                                'Failed to persist wishlist to server',
                                                resp.status
                                            );
                                        }
                                        ...
                                        console.error(
                                            'Error persisting wishlist to server',
                                            e
                                        );
                                    }
                                    ...
                                    // Remove from watchlist if added to wishlist
                                    if (
                                        updated.includes(id) &&
                                        ...
                                        try {
                                            ...
                                            if (r2.status !== 200) {
                                                console.error(
                                                    'Failed to persist watchlist to server',
                                                    r2.status
                                                );
                                            }
                                            ...
                                            console.error(
                                                'Error persisting watchlist to server',
                                                e
                                            );
                                        }
